    ...
    public function rank(): array
    {
        ...
        foreach ($this->phoneNumbers as $phoneNumber) {
            ...
            if (preg_match('/\b(\d+)\1+\b/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 100;
            }
            ...
            if (preg_match('/((\d)123|123(\d)|(\d)234|234(\d)|(\d)345|345(\d)|(\d)456|456(\d)|(\d)567|567(\d)|(\d)678|678(\d)|(\d)789|789(\d)|(\d)890|890(\d)|(\d)987|987(\d)|(\d)876|876(\d)|(\d)765|765(\d)|(\d)654|654(\d)|(\d)543|543(\d)|(\d)432|432(\d)|(\d)321|321(\d)|(\d)210|210(\d))/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 90;
            }
            ...
            // 2 is 0
            if (preg_match('/\d0\d{2}/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 50;
            }

            // 3 is 0
            if (preg_match('/\d{3}0\d/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 45;
            }

            // 1 is 0
            if (preg_match('/0\d{3}/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 25;
            }

            // if contains same number more than once
            if (preg_match('/(\d).*\1/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // * 5309 /song /
            if (str_contains($lastFour, '5309')) {
                $rankedPhoneNumber->rank_phone_number_points += 100;
            }

            // * XX69 /funny
            if (preg_match('/\d{2}69/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 10;
            }

            // * X420 /funny
            if (preg_match('/\d420/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 10;
            }

            // * X865 /song
            if (preg_match('/\d865/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 10;
            }

            // * 865X /song
            if (preg_match('/865\d/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 10;
            }
            ...
            // Patterns (X369)
            if (preg_match('/\d369/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (369X)
            if (preg_match('/369\d/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (X246)
            if (preg_match('/\d246/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (246X)
            if (preg_match('/246\d/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (X468)
            if (preg_match('/\d468/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (468X)
            if (preg_match('/468\d/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (X135)
            if (preg_match('/\d135/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (135X)
            if (preg_match('/135\d/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (X357)
            if (preg_match('/\d357/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (357X)
            if (preg_match('/357\d/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (X579)
            if (preg_match('/\d579/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (579X)
            if (preg_match('/579\d/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (X753)
            if (preg_match('/\d753/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (753X)
            if (preg_match('/753\d/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (X864)
            if (preg_match('/\d864/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (864X)
            if (preg_match('/864\d/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (X642)
            if (preg_match('/\d642/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }

            // Patterns (642X)
            if (preg_match('/642\d/', $lastFour)) {
                $rankedPhoneNumber->rank_phone_number_points += 60;
            }
            ...
            $searchForWords4 = array_keys($this->words, $lastFour);
            $searchForWords5 = array_keys($this->words, $lastFive);
            $searchForWords6 = array_keys($this->words, $lastSix);
            $searchForWords7 = array_keys($this->words, $lastSeven);
            $wordMatch = [];

            if (count($searchForWords4) > 0) {
                $rankedPhoneNumber->rank_phone_number_points += 65;
                $wordMatch = array_merge($wordMatch, $searchForWords4);
            }

            if (count($searchForWords5) > 0) {
                $rankedPhoneNumber->rank_phone_number_points += 65;
                $wordMatch = array_merge($wordMatch, $searchForWords5);
            }

            if (count($searchForWords6) > 0) {
                $rankedPhoneNumber->rank_phone_number_points += 65;
                $wordMatch = array_merge($wordMatch, $searchForWords6);
            }

            if (count($searchForWords7) > 0) {
                $rankedPhoneNumber->rank_phone_number_points += 65;
                $wordMatch = array_merge($wordMatch, $searchForWords7);
            }
